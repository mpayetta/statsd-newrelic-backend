...
var options = {
    ...
    dispatchMetrics: ['gauges', 'counters', 'timers', 'sets', 'counter_rates'],
...
exports.init = function (startup_time, config, events, logger) {
    ...
    if (config.newRelicLog) {
        process.env.NEW_RELIC_LOG = config.newRelicLog;
    }
    if (config.newRelicApp && config.newRelicLicense) {
        process.env.NEW_RELIC_NO_CONFIG_FILE = 'yes';
    }
